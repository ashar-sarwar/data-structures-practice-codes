...
void DjikstraShortestPathFinder(Vertex *vertex,Vertex *S,Vertex *D);
int main()
{
    ...
    DjikstraShortestPathFinder(Vertex,Vertex,Vertex);
...
void DjikstraShortestPathFinder(Vertex *vertex,Vertex *S,Vertex *D)
{
    ...
    for(int i=0;UV!=NULL;i++)
    {
        ...
        cout<<"sORTED";
        displayKedges(UV);
        Vertex *V=findVertex(UV->FisrtVertex,vertex);
        Vertex *U=findVertex(UV->SecondVertex,vertex);
        V->visited=true;
        if(V->minpath<U->minpath)
        {
            U->minpath=V->minpath;
        }
        ArrangePrimsEdges(U,&UV);
        UV=UV->Next;
    }
    cout<<"D->minpath"<<D->minpath;
}
